...
export class SalesOrderHeaderServiceImpl implements SalesOrderHeaderService {
    ...
    public async beforeCreate(
        params: SaledOrderHeader,
    ): Promise<CreationSalesOrderResult> {
        // Produtos
        ...
        if (!products) {
            return {
                hasError: true,
                error: new Error(
                    'Nenhum produto da lista de itens foi encontrado.',
                ),
            };
        }
        ...
        const validationResult = header.validadeCreationPayload({
    ...
    public async afterCreate(
        params: SaledOrderHeaders,
        loggedUser: User,
    ): Promise<void> {
        const headersAsArray = Array.isArray(params)
            ? params
            : ([params] as SaledOrderHeaders);
        const logs: SalesOrderLogModel[] = [];
        for (const header of headersAsArray) {
            const products = (await this.getListOfProduct(
                header,
            )) as ProductModel[];
            ...
            const log = this.getSalesOrderLog(
                salesOrderHeader,
    ...
    private async getListOfProduct(
        params: SaledOrderHeader,
    ): Promise<ProductModel[] | null> {
        const productsIds: string[] = params.items?.map(
            (item: SaledOrderItem) => item.product_id,
        ) as string[];
        return this.productRepository.findByIds(productsIds);
    }

    private getListOfItem(
        params: SaledOrderHeader,
    ...
    private getSalesOrderLog(
        salesOrderHeader: SalesOrderHeaderModel,
